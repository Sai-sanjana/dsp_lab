//quick sort program by taking median of (n/4, middle and 3n/4) as pivot element
#include<stdio.h>
#include<time.h>
#include<stdlib.h>
#include<math.h>
...
{
   int i, j, pivot, temp,size,middle,x,y;
   size = (last-first);
   middle=(first+last)/2;
   x=(first+ (size/4));
   y=((3*size)/4)+first;
    ...
    if (number[middle] < number[x]) 
		swap(&number[middle], &number[x]);
	if (number[y] < number[x])
		swap(&number[y], &number[x]);
	if (number[y] < number[middle])
		swap(&number[y], &number[middle]);
	swap(&number[middle], &number[first]);
...
int main(){
   int i, count, number[n];   
	...
	printf("\n Quick sort by taking Median of (n/4, middle and 3n/4 th element) as pivot with n value as %d\n",n);
