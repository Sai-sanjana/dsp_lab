// quick sort program by taking first element as pivot
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
#define n 10000
...
int partition(int number[n],int first,int last) 
{
   int i, j, pivot, temp;
      pivot=first;
      i=first;
...
int main(){
   int i, count, number[n];   
	...
	printf("\n Quick sort by taking 1st element as pivot with n value as %d\n",n);
